...
class ForationControl(Card):
        ...
        self.content=Container(
            ...
            content=Column(
                [
                    Row(
                        [
                            Text("Foration",text_align=TextAlign.CENTER,color=Colors.BLUE_500)
    ...
    def del_foration(self,e):
        ...
        cursor = conn.cursor()
        cursor.execute("DELETE FROM foration WHERE id=?", (rid,))
        conn.commit()
        conn.close()
    ...
    def updateData(self):
        ...
        if donnees:
            ...
            self.cont.controls.append(Row(
                [
                    Text("Pas de données de FORATION enrégistré")
                ],alignment=MainAxisAlignment.CENTER))
        try:
    ...
    def showUpdateData(self,e):
        if self.donnees==[]:
            cont=ForationForm(page=self.page,local_id=self.local_id, formcontrol=self)
        else:
            cont=ForationUpdateForm(page=self.page,local_id=self.local_id, donnees=self.donnees[0] , formcontrol=self)
        self.dlg_modal = AlertDialog(
            modal=True,
            title=Text("Info Foration"),
            content=cont,
            actions=[
