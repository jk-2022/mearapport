...
class PompageControl(Card):
    def __init__(self, page: Page, donnees, local_id):
        super().__init__()
        self.page=page
        ...
        self.content=Container(
            border_radius=10,
            border=border.all(1,Colors.PINK_500),
            expand=True,
            content=Column(
                [
                    Row(
                        [
                            Text("Pompage",text_align=TextAlign.CENTER,color=Colors.PINK_500)
    ...
    def showDelete(self,e):
        self.dlg_modal = AlertDialog(
            ...
            actions=[
                ...
                TextButton("Oui", on_click=self.del_foration, icon=Icons.DELETE, icon_color=Colors.RED),
            ],
    ...
    def del_foration(self,e):
        rid=int(self.donnees[0]['id'])
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("DELETE FROM pompage WHERE id=?", (rid,))
        conn.commit()
        conn.close()
    ...
    def updateData(self):
        ...
        donnees=all_data['pompage']
        self.donnees=donnees
        self.cont.controls.clear()
        if donnees:
            self.delete_btn.visible=True
            list_item=['id','localisation_id','created_at']
            ...
            self.cont.controls.append(Row(
                [
                    Text("Pas de données de POMPAGE enrégistré")
                ],alignment=MainAxisAlignment.CENTER))
        try:
    ...
    def showUpdateData(self,e):
        if self.donnees==[]:
            cont=PompageForm(page=self.page,local_id=self.local_id, formcontrol=self)
        else:
            cont=PompageUpdateForm(page=self.page,local_id=self.local_id, donnees=self.donnees[0] , formcontrol=self)
        self.dlg_modal = AlertDialog(
            modal=True,
            title=Text("info Pompage"),
            content=cont,
            actions=[
